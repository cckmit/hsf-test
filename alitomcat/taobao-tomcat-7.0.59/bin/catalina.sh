...
# set CATALINA_LOGS from CATALINA_BASE if not already set
if [ -z "$CATALINA_LOGS" ]; then
  CATALINA_LOGS="$CATALINA_BASE"/logs
fi

if [ -z "$CATALINA_OUT" ] ; then
  CATALINA_OUT="$CATALINA_LOGS"/catalina.out
fi
...
if [[ "$JAVA_OPTS" == *-Dhotcode.* ]]; then
    JAVA_OPTS="$JAVA_OPTS -javaagent:${CATALINA_HOME}/tools/hotcode2.jar"
fi
